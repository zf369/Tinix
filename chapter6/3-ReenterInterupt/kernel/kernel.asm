...
hwint00:        ; Interrupt routine for irq 0 (the clock).
    ...
    ; >>>>>>>>>>>>>> esp指向进程表处 <<<<<<<<<<<<<<<<<<<<
    sub    esp, 4  ; 跳过返回地址，原因现在不明

    pushad      ; ┓
    push   ds   ; ┃
    push   es   ; ┣ 保存中断前原寄存器值到进程表中，当前esp指向进程表处
    push   fs   ; ┃
    push   gs   ; ┛
    ...
    ; 使用call，打印clock_int_msg，测试堆栈是否切换完成
    push   clock_int_msg
    call   disp_str
    add    esp, 4

    ; call delay, 保证中断执行时间足够长，产生中断嵌套
    ;push   10
    ;call   delay
    ;add    esp, 4
    ...
    ; >>>>>>>>>>>>>> 离开内核栈，esp指向接下来要切换的进程表 <<<<<<<<<<<<<<<<<<<<
    mov    esp, [p_proc_ready]

    ; 下次时钟中断发生时，esp将指向的进程表位置
    lea    eax, [esp + P_STACKTOP]
    mov    dword [tss + TSS3_S_SP0], eax

.re_enter:    ; 如果 k_reenter != 0, 会jmp到这里
