...
LABEL_CALL_GATE_TEST: Gate  SelectorCodeDest,         0,        0,  DA_386CGate + DA_DPL0 ; DPL=0的调用门
...
LABEL_STACK:
	; zf: 现在堆栈是512B，测试一下将堆栈设置的特别小，call函数的时候是否会报错???
...
.2:     ; 显示PMMessage完毕
        ...
        ; zf: 测试后面的offset不为0是否可以？？
        ; 测试结果: 修改以后不影响，应该是调用门描述符中的offset才是生效的offset
        call    SelectorCallGateTest:9999    ; 测试调用门（无特权级变换），将打印字母 'C'。
